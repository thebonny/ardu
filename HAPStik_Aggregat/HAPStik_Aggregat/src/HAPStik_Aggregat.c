...
------------------------------------------------------------------------------------------------------------------------------
 15	14:20	- 3 Kabel zum DUE
 10			- 2 Kabel Versorgung 3,3V am Leistungsteil loopen

 15			- 3 Kabel zum Motor (Stiftleiste)
 10			- 2 Kabel Versorgung VCC_Motor am Leistungsteil loopen

 10			- Steckverbinder Aggregat anschauen, ... evtl. Referenz an Poti2 anlegen?!
 10			- Poti2 an ADC-Eingang A1
			
 20			- Poti_Sollwertvorgabe an ADC-Eingang A2 -> Verstärkung prüfen 
---
 90
			- cool wäre wenn der Stick auf einer Kreisbahn liefe
				- dazu zwei um 90° verschobene Sinusausgaben als Winkelwert
					-> MOTOR1: Auslenkung: sin(0°)			* POT_U_ver_MAX1
					-> MOTOR1: Auslenkung: sin(0° + 90°)	* POT_U_hor_MAX1				
...
//	ANFANG **********************************************     Includes       *************************************************
	...
	#include "delay.h"					// z.B.	delay_us(564)	-> warte 564us
										...
										//		delay_s(2)		-> warte 2s
	#include <uart_serial.h>			// UART		z.B. printf("MEC22_STEPS:   %i\r\n", ENC1_C);
...
// SnapShot der ADC-Werte in der Print-Ausgabe erstellt	
	...
	volatile	float	WKL_OFF_1 =		5.0;								// Winkel_Offset für Poti_MOTOR1 Vertikal	(Nullposition)	-> "+" Stick wandert nach oben
	volatile	float	WKL_OFF_2 =		-16.0;								// Winkel_Offset für Poti_MOTOR2 Horizontal	(Nullposition)	-> "+" Stick wandert nach links
...
//	Debug Pin C.24 gibt es nicht mehr, ist jetzt eine PWM-Leitung
//	Debug_Pin C.24 = 1
//	REG_PIOC_SODR	= REG_PIOC_SODR		|	0x01000000u;
//	Debug_Pin C.24 = 0
//	REG_PIOC_CODR	= REG_PIOC_CODR		|	0x01000000u;
...
*/
	...
	if (af_count_i == 10)										// 10 ADC-Werte wurden addiert, 1ms ist vorbei
	{
...
//		AF_A0_f = 0.5 + SUM_AF_i / 10.0;						// hier 0,5 zur Rundung dazu addieren
		...
		AF_A1_f = SUM_AF_i_2 / 10.0;							// da ja alle Positionen beim Start des Sticks eingemessen werden
...
//	Ausgabe an den PWM-VController
	...
	REG_PWM_CDTYUPD0 = (1 - PWMu1) * 4200;
	REG_PWM_CDTYUPD1 = (1 - PWMv1) * 4200;	
	REG_PWM_CDTYUPD2 = (1 - PWMw1) * 4200;	

	REG_PWM_CDTYUPD3 = (1 - PWMu2) * 4200;
	REG_PWM_CDTYUPD4 = (1 - PWMv2) * 4200;
	REG_PWM_CDTYUPD5 = (1 - PWMw2) * 4200;
...
//	- Kanal 2:
//		- center aligned, Start mit high-Level, Dead Time disabled
	REG_PWM_CMR5 = REG_PWM_CMR5		|	0x00010300u;
...
*/
	...
	REG_PWM_CDTY3 = REG_PWM_CDTY3	|	0x00001068u;		// 4200
	REG_PWM_CDTY4 = REG_PWM_CDTY4	|	0x00001068u;		// 4200
	REG_PWM_CDTY5 = REG_PWM_CDTY5	|	0x00001068u;		// 4200
...
*/
//	Impuls auf der EventLine wird mit jedem 1. PWM-Referenzimpuls erzeugt (1  x 100us = 100us)
...
//	Leistungsfaktoren (0...1) MOTOR1, MOTOR2
	...
	TD1 = 2;											// Zeitverzögerung1
	TD2 = 100;											// Zeitverzögerung2
...
//	MOTOR1 -------------------------------------------------------------------------------------------------------------------
//	REGLER POSITION
//	mySetpoint2 = 2430;								// Führungsgröße manuell festlegen: 2430 -> 0° (mech.)

//	ACHTUNG
	mySetpoint2_1 = 400 + SS_ADC_A2_i;
	
//	mySetpoint2_1	= 800 + 0.6 * SS_ADC_A2_i;			// Führungsgröße MOTOR1 durch Poti "Masterstick" festlegen
//	mySetpoint2_1	= 2418;
	
	outMax2_1		=  0.99f;						// Max Drehmoment MOTOR1
	outMin2_1		= -0.99f;						// Min Drehmoment MOTOR1

	kp2_1			= 0.001;						// Koeffizienten P-, I-, D-Anteil
	ki2_1			= 0.0;
	kd2_1			= 0.0;

/*
	kp2_1			= 0.003;						// Koeffizienten P-, I-, D-Anteil
	ki2_1			= 0.0000025;
	kd2_1			= 0.003;
*/	
...
//	Poti lesen, erfolgt in der ADC-Interrupt Routine
			if (ADC_ISR_CNT % 10000 == 0)			// 10000 x 100us = 1s
			{
//				PRINT_ADC();
		
				printf("|AF_A0_i: %10d|DWE1   : %10s|LF1    : %10s|MAX1   : %10s|MIN1   : %10s|NULL1  : %10s|\r\n",
				AF_A0_i, doubleToString(s1, DWE1), doubleToString(s2, LF1), doubleToString(s3, POT_U_ver_MAX1), doubleToString(s4, POT_U_ver_MIN1), doubleToString(s5, POT_U_ver_NULL1));							
...
//	MOTOR2 -------------------------------------------------------------------------------------------------------------------
	mySetpoint2_2 = 400 + SS_ADC_A2_i;				// Führungsgröße MOTOR2 durch Poti "Masterstick" festlegen	
//	mySetpoint2_2	= 2304;
	
	outMax2_2		=  0.99;						// Max Drehmoment MOTOR2
	outMin2_2		= -0.99;						// Min Drehmoment MOTOR2


	kp2_2			= 0.001;						// Koeffizienten P-, I-, D-Anteil
	ki2_2			= 0.0;
	kd2_2			= 0.0;
...
//	PRINT
//	Poti lesen, erfolgt in der ADC-Interrupt Routine
			if (ADC_ISR_CNT_1ms % 1000 == 0)			// 1000 x 1ms = 1s
			{
//				PRINT_ADC();
				...
				printf("|AF_A1_i: %10d|DWE2   : %10s|LF2    : %10s|MAX2   : %10s|MIN2   : %10s|NULL2  : %10s|\r\n\n",
				AF_A1_i, doubleToString(s1, DWE2), doubleToString(s2, LF2), doubleToString(s3, POT_U_ver_MAX2), doubleToString(s4, POT_U_ver_MIN2), doubleToString(s5, POT_U_ver_NULL2));							
			}		
*/
